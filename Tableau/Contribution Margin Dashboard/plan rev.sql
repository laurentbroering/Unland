with
	...
	, adj_payments as (
		...
		, case
			...
			else subscribed_at + interval '8 days' > canceled_at and subscribed_at <= canceled_at end withdrawn_revenue

		-- Calcula o valor da receita do período
		, case when reason = 'subscription' then coalesce(total_price / 12.00, orders.preco) else payments.value end as value
		...
		where payments.reason in ('subscription', 'monthly_fee', 'overdue_monthly_fee'))
	...
	, payments_by_period as (
		...
		, partner_share
		, insurer_share
		, coalesce(withdrawn_revenue,false) withdrawn_revenue
		, value
		...
		group by start_interval, end_interval,
			id, plan_id, direct_channel, subscribed_at, canceled_at, order_id, created_at, partner_id, pitzi_share, partner_share, insurer_share, withdrawn_revenue, value, plan_value, admin_value, pitzi_plan_value, partner_plan_value, insurer_plan_value
	...
	select
		...
		, count(id) revenue_count -- Número de planos vendidos
		...
		, - sum(case when withdrawn_revenue then 1 else 0 end) withdrawn_revenue_count

		, sum(value) total_revenue -- Total de vendas
			, sum(plan_value) plan_revenue -- Total de vendas de planos de proteção
				, sum(plan_value) * avg(pitzi_share) pitzi_plan_revenue -- Share de vendas de planos de proteção da Pitzi
				, sum(plan_value) * avg(partner_share) partner_plan_revenue -- Share de vendas de planos de proteção do parceiro
			, sum(admin_value) rfq_revenue -- Total de vendas de planos de RFQ (inclui IOF)
				, sum(admin_value) * 0.0738 iof_rfq_revenue -- IOF sobre planos de RFQ
				, sum(admin_value) * avg(pitzi_share) pitzi_rfq_revenue -- Share de vendas de planos de RFQ da Pitzi (sem IOF)
				, sum(admin_value) * avg(partner_share) partner_rfq_revenue -- Share de vendas de planos de RFQ do parceiro (sem IOF)
				, sum(admin_value) * avg(insurer_share) insurer_rfq_revenue -- Share de vendas de planos de RFQ da seguradora (sem IOF)

		, - sum(plan_value) * 9.25 / 100.00 as plan_revenue_pis_cofins -- PIS/COFINS sobre planos de proteção
			, - sum(plan_value) * avg(pitzi_share) * 9.25 / 100.00 as pitzi_plan_revenue_pis_cofins -- PIS/COFINS sobre planos de proteção pagos pela Pitzi
			, - sum(plan_value) * avg(partner_share) * 9.25 / 100.00 as partner_plan_revenue_pis_cofins -- PIS/COFINS sobre planos de proteção pagos pelo parceiro

		, - sum(admin_value) * (1 - 0.0738) * 9.25 / 100.00 as rfq_revenue_pis_cofins -- PIS/COFINS sobre planos de RFQ
			, - sum(admin_value) * avg(pitzi_share) * 9.25 / 100.00 as pitzi_rfq_revenue_pis_cofins -- PIS/COFINS sobre planos de RFQ pagos pela Pitzi
			, - sum(admin_value) * avg(partner_share) * 9.25 / 100.00 as partner_rfq_revenue_pis_cofins -- PIS/COFINS sobre planos de RFQ pagos pelo parceiro
			, - sum(admin_value) * avg(insurer_share) * 9.25 / 100.00 as insurer_rfq_revenue_pis_cofins -- PIS/COFINS sobre planos de RFQ pagos pela seguradora

		, - sum(admin_value) * (1 - 0.0738) * 5.00 / 100.00 as rfq_revenue_iss -- ISS sobre planos de RFQ
			, - sum(admin_value) * avg(pitzi_share) * 5.00 / 100.00 as pitzi_rfq_revenue_iss -- ISS sobre planos de RFQ pagos pela Pitzi
			, - sum(admin_value) * avg(partner_share) * 5.00 / 100.00 as partner_rfq_revenue_iss -- ISS sobre planos de RFQ pagos pelo parceiro
			, - sum(admin_value) * avg(insurer_share) * 5.00 / 100.00 as insurer_rfq_revenue_iss -- ISS sobre planos de RFQ pagos pela seguradora
